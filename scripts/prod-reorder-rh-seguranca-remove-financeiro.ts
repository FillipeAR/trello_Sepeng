/**
 * Mesma migração de scripts/reorder-rh-seguranca-remove-financeiro.ts, mas
 * pensada pra rodar contra produção (Neon), cujo schema real ainda não tem
 * as colunas da rodada de rate limiting/cadastro (User.emailVerifiedAt,
 * LoginAttempt, EmailVerificationToken — só local por enquanto). Por isso
 * a resolução do actor usa `select` explícito em vez de `include: { user: true }`,
 * pra não tentar ler uma coluna que não existe em produção ainda.
 *
 * Uso: DATABASE_URL="<neon prod>" npx tsx scripts/prod-reorder-rh-seguranca-remove-financeiro.ts
 */
